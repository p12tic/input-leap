...
std::string error_code_to_string_windows(DWORD err)
{
    ...
    if (FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS |
                      ...
                      0, err, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR) &cmsg, 0,
                      nullptr) == 0) {
        cmsg = nullptr;
...
std::string error_code_to_string_winsock(int err)
{
    ...
    static const struct {
        ...
        /* 10035 */
        {WSAEWOULDBLOCK, "Socket is marked as non-blocking and no connections are present or the "
        ...
        /* 10055 */
        {WSAENOBUFS, "No buffer space is available or a buffer deadlock has occurred. The socket "
                     "cannot be created"},
        ...
        /* 10091 */
        {WSASYSNOTREADY, "Underlying network subsystem is not ready for network communication"},
        /* 10092 */
        {WSAVERNOTSUPPORTED,
         "The version of WinSock API support requested is not provided in this implementation"},
