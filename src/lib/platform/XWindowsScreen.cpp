...
XWindowsScreen::XWindowsScreen(IXWindowsImpl* impl, const char* displayName, bool isPrimary,
    ...
    m_events(events)
{
    ...
    try {
        ...
        saveShape();
        m_window = openWindow();
        m_screensaver = new XWindowsScreenSaver(m_impl, m_display, m_window, get_event_target(),
                                                events);
        m_keyState = new XWindowsKeyState(m_impl, m_display, m_xkb, events, m_keyMap);
        LOG_DEBUG("screen shape: %d,%d %dx%d %s", m_x, m_y, m_w, m_h,
                  m_xinerama ? "(xinerama)" : "");
    ...
    m_events->set_buffer(std::make_unique<XWindowsEventQueueBuffer>(m_impl, m_display, m_window,
                                                                    m_events));
}
...
void XWindowsScreen::enter()
{
    ...
    CARD16 powerlevel;
    BOOL enabled;
    if (m_impl->DPMSQueryExtension(m_display, &dummy, &dummy) && m_impl->DPMSCapable(m_display) &&
...
void XWindowsScreen::getShape(std::int32_t& x, std::int32_t& y, std::int32_t& w,
                              std::int32_t& h) const
{
    x = m_x;
    y = m_y;
...
void XWindowsScreen::getCursorPos(std::int32_t& x, std::int32_t& y) const
{
    ...
    unsigned int mask;
    if (m_impl->XQueryPointer(m_display, m_root, &root, &window, &mx, &my, &xWindow, &yWindow,
                              &mask)) {
        x = mx;
        y = my;
    } else {
...
void XWindowsScreen::warpCursor(std::int32_t x, std::int32_t y)
{
    ...
    XEvent event;
    while (m_impl->XCheckMaskEvent(m_display,
...
std::uint32_t XWindowsScreen::registerHotKey(KeyID key, KeyModifierMask mask)
{
    ...
    bool err = false;
    {
        ...
        if (key == kKeyNone) {
            ...
            for (size_t j = 0; j < sizeof(s_hotKeyModifiers) / sizeof(s_hotKeyModifiers[0]) && !err;
                 ++j) {
                ...
                const KeyCode* modifiermap = modKeymap->modifiermap +
                                             index * modKeymap->max_keypermod;
                for (int k = 0; k < modKeymap->max_keypermod && !err; ++k) {
                    KeyCode code = modifiermap[k];
                    if (modifiermap[k] != 0) {
                        m_impl->XGrabKey(m_display, code, modifiers2, m_root, False, GrabModeAsync,
                                         GrabModeAsync);
                        if (!err) {
        ...
        // those.
            ...
            for (auto j = keycodes.begin(); j != keycodes.end() && !err; ++j) {
                for (size_t i = 0; i < (1u << numToggleModifiers); ++i) {
                    ...
                    // add grab
                    m_impl->XGrabKey(m_display, *j, tmpModifiers, m_root, False, GrabModeAsync,
                                     GrabModeAsync);
                    if (!err) {
    ...
    if (err) {
        ...
        LOG_WARN("failed to register hotkey %s (id=%04x mask=%04x)",
                 inputleap::KeyMap::formatKey(key, mask).c_str(), key, mask);
        return 0;
    }

    LOG_DEBUG("registered hotkey %s (id=%04x mask=%04x) as id=%d",
              inputleap::KeyMap::formatKey(key, mask).c_str(), key, mask, id);
    return id;
}
...
bool XWindowsScreen::isAnyMouseButtonDown(std::uint32_t& buttonID) const
{
    ...
    unsigned int state;
    if (m_impl->XQueryPointer(m_display, m_root, &root, &window, &xRoot, &yRoot, &xWindow, &yWindow,
                              &state)) {
...
void XWindowsScreen::fakeMouseButton(ButtonID button, bool press)
{
    ...
    if (xButton > 0 && xButton < 11) {
        m_impl->XTestFakeButtonEvent(m_display, xButton, press ? True : False, CurrentTime);
...
Display* XWindowsScreen::openDisplay(const char* displayName)
{
    ...
    if (!m_isPrimary) {
        ...
        if (!m_impl->XQueryExtension(display, XTestExtensionName, &majorOpcode, &firstEvent,
                                     &firstError)) {
...

    {
        ...
        if (m_impl->XkbLibraryVersion(&major, &minor)) {
            ...
            if (m_impl->XkbQueryExtension(display, &opcode, &m_xkbEventBase, &firstError, &major,
                                          &minor)) {
                m_xkb = true;
                m_impl->XkbSelectEvents(display, XkbUseCoreKbd, XkbMapNotifyMask, XkbMapNotifyMask);
                m_impl->XkbSelectEventDetails(display, XkbUseCoreKbd, XkbStateNotifyMask,
...
void XWindowsScreen::saveShape()
{
    ...
    if (m_impl->XineramaQueryExtension(m_display, &eventBase, &errorBase) &&
        ...
        screens = reinterpret_cast<XineramaScreenInfo*>(XineramaQueryScreens(m_display,
                                                                             &numScreens));
...
Window XWindowsScreen::openWindow() const
{
    ...
    if (m_isPrimary) {
        ...
        // the whole screen.
        attr.event_mask = PointerMotionMask | ButtonPressMask | ButtonReleaseMask | KeyPressMask |
                          KeyReleaseMask | KeymapStateMask | PropertyChangeMask;
        x = m_x;
        y = m_y;
        w = m_w;
        h = m_h;
    ...
    Window window =
        m_impl->XCreateWindow(m_display, m_root, x, y, w, h, 0, 0, InputOnly,
...
void XWindowsScreen::openIM()
{
    ...
    // only at the moment.
    XIMStyles* styles;
    if (m_impl->XGetIMValues(im, XNQueryInputStyle, &styles) != nullptr || styles == nullptr) {
...
void XWindowsScreen::handle_system_event(const Event& event)
{
    ...
    if (m_isPrimary) {
        if (xevent->type == KeyRelease) {
            ...
            XEvent xevent2;
            isRepeat = (m_impl->XCheckIfEvent(m_display, &xevent2, &XWindowsScreen::findKeyEvent,
    ...
    if (m_xi2detected) {
        ...
        if (m_impl->XGetEventData(m_display, cookie) && cookie->type == GenericEvent &&
            ...
            if (cookie->evtype == XI_RawMotion) {
                ...
                unsigned int msk;
                xmotion.same_screen = m_impl->XQueryPointer(m_display, m_root, &xmotion.root,
                                                            &xmotion.subwindow, &xmotion.x_root,
                                                            &xmotion.y_root, &xmotion.x, &xmotion.y,
                                                            &msk);
                onMouseMove(xmotion);
    ...
    case SelectionNotify:
        ...
        if (xevent->xselection.property != None) {
            m_impl->XDeleteProperty(m_display, xevent->xselection.requestor,
...
void XWindowsScreen::onKeyPress(XKeyEvent& xkey)
{
    ...
    if (key != kKeyNone) {
        ...
        // handle key
        m_keyState->sendKeyEvent(get_event_target(), true, false, key, mask, 1, keycode);

        // do fake release if this is a fake press
        if (isFake) {
            m_keyState->sendKeyEvent(get_event_target(), false, false, key, mask, 1, keycode);
        }
    } else {
...
void XWindowsScreen::onKeyRelease(XKeyEvent& xkey, bool isRepeat)
{
    ...
    if (key != kKeyNone) {
        ...
        if (!isRepeat) {
            ...
            m_keyState->sendKeyEvent(get_event_target(), false, true, key, mask, 1, keycode);
        }
    }
}
...
void XWindowsScreen::onMouseRelease(const XButtonEvent& xbutton)
{
    ...
    if (button != kButtonNone) {
        ...
        sendEvent(EventType::PRIMARY_SCREEN_WHEEL, create_event_data<WheelInfo>(WheelInfo{0, 120}));
    } else if (xbutton.button == 5) {
        // wheel backward (toward user)
        sendEvent(EventType::PRIMARY_SCREEN_WHEEL, create_event_data<WheelInfo>(WheelInfo{0, -120}));
    } else if (xbutton.button == 6) {
        // wheel left
        sendEvent(EventType::PRIMARY_SCREEN_WHEEL, create_event_data<WheelInfo>(WheelInfo{-120, 0}));
...
Cursor XWindowsScreen::createBlankCursor() const
{
    ...
    Cursor cursor = m_impl->XCreatePixmapCursor(m_display, bitmap, bitmap, &color, &color, 0, 0);
...
ButtonID XWindowsScreen::mapButtonFromX(const XButtonEvent* event) const
{
    ...
    case 1:
    case 2:
    case 3: // kButtonLeft, Middle, Right
        return static_cast<ButtonID>(button);
    case 4:
    case 5:
    case 6:
    case 7: // scroll up, down, left, right -- ignored here
        return kButtonNone;
...
{
    unsigned int event_mask = ButtonPressMask | ButtonReleaseMask | EnterWindowMask |
    ...
    do {
        // keyboard first
        do {
            result = m_impl->XGrabKeyboard(m_display, m_window, True, GrabModeAsync, GrabModeAsync,
                                           CurrentTime);
        ...
        result = m_impl->XGrabPointer(m_display, m_window, False, event_mask, GrabModeAsync,
...
{
    int event, error;
    return m_impl->XQueryExtension(m_display, "XInputExtension", &xi_opcode, &event, &error);
}

bool XWindowsScreen::detectXwayland()
{
    int opcode, event, error;
    return m_impl->XQueryExtension(m_display, "XWAYLAND", &opcode, &event, &error);
}
