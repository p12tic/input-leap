...
PortalRemoteDesktop::PortalRemoteDesktop(EiScreen* screen, IEventQueue* events) :
    screen_(screen), events_(events), portal_(xdp_portal_new())
{
...
static inline void xdp_portal_create_remote_desktop_session_full(
    ...
    XdpCursorMode cursor_mode, XdpPersistMode _unused1, const char* _unused2,
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer data)
{
    xdp_portal_create_remote_desktop_session(portal, devices, outputs, flags, cursor_mode,
...
gboolean PortalRemoteDesktop::init_remote_desktop_session()
{
    ...
    xdp_portal_create_remote_desktop_session_full(
        portal_, static_cast<XdpDeviceType>(XDP_DEVICE_POINTER | XDP_DEVICE_KEYBOARD),
