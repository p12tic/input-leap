...
Server::Server(Config& config, PrimaryClient* primaryClient, inputleap::Screen* screen,
    ...
    m_args(args)
{
    ...
    m_events->add_handler(EventType::PRIMARY_SCREEN_WHEEL, m_primaryClient->get_event_target(),
...
Server::~Server()
{
    ...
    m_events->remove_handler(EventType::PRIMARY_SCREEN_MOTION_ON_PRIMARY,
                             m_primaryClient->get_event_target());
    m_events->remove_handler(EventType::PRIMARY_SCREEN_MOTION_ON_SECONDARY,
                             m_primaryClient->get_event_target());
    m_events->remove_handler(EventType::PRIMARY_SCREEN_WHEEL, m_primaryClient->get_event_target());
    m_events->remove_handler(EventType::PRIMARY_SCREEN_SAVER_ACTIVATED,
                             m_primaryClient->get_event_target());
    m_events->remove_handler(EventType::PRIMARY_SCREEN_SAVER_DEACTIVATED,
                             m_primaryClient->get_event_target());
...
void Server::switchScreen(BaseClientProxy* dst, std::int32_t x, std::int32_t y, bool forScreensaver)
{
    ...
    LOG_INFO("switch from \"%s\" to \"%s\" at %d,%d", getName(m_active).c_str(),
             getName(dst).c_str(), x, y);
                                     ...
                                     std::int32_t& y) const
{
    ...
    for (;;) {
        ...
        if (index != m_clients.end()) {
            LOG_DEBUG2("\"%s\" is on %s of \"%s\" at %f", dstName.c_str(), Config::dirName(dir),
                       srcName.c_str(), t);
        ...
        LOG_DEBUG2("ignored \"%s\" on %s of \"%s\"", dstName.c_str(), Config::dirName(dir),
                   srcName.c_str());
        srcName = dstName;
...
void Server::startSwitchWait(std::int32_t x, std::int32_t y)
{
    stopSwitchWait();
    m_switchWaitX = x;
    m_switchWaitY = y;
...
void Server::handle_clipboard_grabbed(const Event& event, BaseClientProxy* grabber)
{
    ...
    LOG_INFO("screen \"%s\" grabbed clipboard %d from \"%s\"", getName(grabber).c_str(), info.m_id,
...
void Server::handle_toggle_screen_event(const Event& event)
{
    (void) event;
...
void Server::handle_keyboard_broadcast_event(const Event& event)
{
    ...
    // enter new state
    if (newState != m_keyboardBroadcasting || info.screens_ != m_keyboardBroadcastingScreens) {
        m_keyboardBroadcasting = newState;
        m_keyboardBroadcastingScreens = info.screens_;
        LOG_DEBUG("keyboard broadcasting %s: %s", m_keyboardBroadcasting ? "on" : "off",
...
void Server::onClipboardChanged(BaseClientProxy* sender, ClipboardID id, std::uint32_t seqNum)
{
    ...
    if (seqNum < clipboard.m_clipboardSeqNum) {
        LOG_INFO("ignored screen \"%s\" update of clipboard %d (missequenced)",
    ...
    if (data.size() > m_maximumClipboardSize) {
        LOG_NOTE("not updating clipboard because it's over the size limit (%zi KB) configured by "
                 "the server",
                 m_maximumClipboardSize);
        return;
    }
    if (data == clipboard.m_clipboardData) {
        LOG_DEBUG("ignored screen \"%s\" update of clipboard %d (unchanged)",
                  clipboard.m_clipboardOwner.c_str(), id);
        return;
    }
...
void Server::onKeyDown(KeyID id, KeyModifierMask mask, KeyButton button, const char* screens)
{
...
bool Server::onMouseMovePrimary(std::int32_t x, std::int32_t y)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (isSwitchOkay(newScreen, dir, x, y, xc, yc)) {
            if (m_args.m_enableDragDrop && m_screen->isDraggingStarted() && m_active != newScreen &&
...
void Server::closeClient(BaseClientProxy* client, const char* msg)
{
    ...
    m_events->add_handler(EventType::TIMER, timer,
                          [this, client](const auto& e) { handle_client_close_timeout(client); });
...
void Server::forceLeaveClient(BaseClientProxy* client)
{
    ...
    if (active == client) {
        ...
        // disconnected.
        LOG_INFO("jump from \"%s\" to \"%s\" at %d,%d", getName(active).c_str(),
...
Server::ClipboardInfo::ClipboardInfo() :
    m_clipboard(), m_clipboardData(), m_clipboardOwner(), m_clipboardSeqNum(0)
{
    // do nothing
}
