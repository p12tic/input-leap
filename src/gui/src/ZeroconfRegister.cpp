...
ZeroconfRegister::ZeroconfRegister(QObject* parent) : QObject(parent), m_DnsServiceRef(nullptr) {}
...
void ZeroconfRegister::registerService(const ZeroconfRecord& record, quint16 servicePort)
{
    if (m_DnsServiceRef) {
    ...
    DNSServiceErrorType err = DNSServiceRegister(
        &m_DnsServiceRef, kDNSServiceFlagsNoAutoRename, 0, record.serviceName.toUtf8().constData(),
        record.registeredType.toUtf8().constData(),
        record.replyDomain.isEmpty() ? nullptr : record.replyDomain.toUtf8().constData(), nullptr,
        bigEndianPort, 0, nullptr, registerService, this);
